        ...
        if len(joints_src)==0 or len(joints_dst)==0: # if limb type has no candidate (peak)
            ...
            for i, joint_src in enumerate(joints_src):
                for j, joint_dst in enumerate(joints_dst):
                    ...
                    criteria1 = (np.count_nonzero(score_inter_pts>0.6) > 0.8*num_inter_points) # no. score > threshold at least 40%
...
def group_limbs_same_idcard(connected_limbs, joint_list):
    ...
    for idcard_id, idcard_info in enumerate(idcard_to_joint_assoc):
        if idcard_info[-1] < 4 or idcard_info[-2]/idcard_info[-1] < 0.2: # not enough 4 corner
...
def paf_to_idcard(heatmap, paf):
    ...
    for idcard_id, idcard_info in enumerate(idcard_to_joint_assoc):
        ...
        idcard = np.zeros((4, 2), dtype=np.float32)
        for i in range(NUM_JOINTS):
            joint = joints[i]
            if joint==-1: #not found joint
                continue     
            peak_coord = joints_list[int(joint)][:2]
            idcard[i] = peak_coord[:2]
        idcards.append(idcard)
    return idcards


# idcards = []
# for idcard_id, idcard_info in enumerate(idcard_to_joint):
#     kps = idcard_info[:n_kps]
#     idcard = np.empty((4, 2), dtype=np.float32)
...
#             idcard[i] = [-1, -1]
#     print(idcard)
# #     idcard = np.array(idcard).reshape(n_kps, 2)
#     idcards.append(idcard)

idcards = paf_to_idcard(heatmap, paf)
