...
from utils.pafprocess_classes import paf_to_idcard
from utils.utils import paf2im
import os
import argparse
...
def show_paf(paf):
    ...
    paf = np.sum(paf, axis=2)
    paf_im = paf2im(paf)
    return paf_im.astype(np.uint8)
...
def inference_single_image(sess, im):
    ...
    ifr_time = time.time() - start
    #print(hm.shape, paf.shape, off.shape)
    hm = hm[0]
    paf = paf[0]
    off = off[0]
    
    # for i in range(4):
    #     paf_classes = paf[:, :, 2*i: 2*(i+1)]
    #     #if i==1:
    #         #paf_classes = paf_classes[40:44, 116: 120, :]
    #         #print(paf_classes[:, :, 0])
    #         #print(paf_classes[:, :, 1])
    #     paf_im = paf2im(paf_classes)
    #     cv2.imwrite('{}.png'.format(i), paf_im)
        #break
    start = time.time()
    results = paf_to_idcard(hm, paf, off, ratio=4)
    ppc_time = time.time() - start
    print('TIME', ifr_time, ppc_time)
    #print(results)
    for k in results.keys():
        for i in range(len(results[k]['pts'])):
            results[k]['pts'][i][:8][::2] -= pad_x 
            results[k]['pts'][i][:8][1::2] -= pad_y
            results[k]['pts'][i][:8] /= ratio
    hm = sigmoid(hm)
    hm_show = show_hm(hm)
...
im_paths = [path for path in im_paths if 'CMT' not in path]
for path in im_paths:
    ...
    for key, value in results.items():
        idcards = value['pts']
        missing_corners = value['missing_corners']
        if len(idcards)>0:
            cv2.putText(im, str(key), (10, 100), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 255), 2)
